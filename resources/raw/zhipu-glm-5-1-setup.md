...
<Tabs>
  <Tab title="Windows">
    找到配置文件 `~/.claude/settings.json`，如下图所示：
    ![Description](https://cdn.bigmodel.cn/markdown/1774605846629image.png?attname=image.png)
  </Tab>
  ...
  <Tab title="macOS">
    ...
    * 方法二：访达 → 前往文件夹 → 输入 `~/.claude/settings.json` 找到此配置文件编辑。
    ...
    ![Description](https://cdn.bigmodel.cn/markdown/1774606073746image.png?attname=image.png)
  </Tab>
</Tabs>

或者可以在Claude Code中询问：

![Description](https://cdn.bigmodel.cn/markdown/1774606110492image.png?attname=image.png)
...
![Description](https://cdn.bigmodel.cn/markdown/1774606248371image.png?attname=image.png)
...
找到配置文件 `~/.openclaw/openclaw.json`，如下图所示：

![Description](https://cdn.bigmodel.cn/markdown/1774606346217image.png?attname=image.png)

在 `~/.openclaw/openclaw.json` 文件中的 `models.providers.zai.models` 数组中添加 `GLM-5.1` 模型，在最后一个模型后面添加：注意 JSON 格式数组里面加逗号
...
修改完成后重启网关：`openclaw gateway restart` , 这样就能直接使用 GLM-5.1 模型了！

终端执行 `openclaw tui` 进入对话可以看到我们正在使用 GLM-5.1 模型。
...
![Description](https://cdn.bigmodel.cn/markdown/177461592642611.jpeg?attname=11.jpeg)
