...
### 6.2.1 Maestro VIP 价格信息（2026-06-12 二手收集，⚠️ 未官方核查）

**凯戈通过 AI 助手聚合查到的二手价格**（来源：36kr / 知乎 / ai-indeed / 淘宝商家页 — 全部非官方）：
...
| 连续包月 / 季度折算 | ~433.3 元/月 |
| 年付原价 | 5,199 元/年 |
| 新用户首年特惠 | 2,599 ~ 3,099 元/年 |

**Maestro VIP 权益（二手）**：
- 月度积分：**15,000 点**（vs 凯戈 standard 月度 1,080 点 = **14 倍**）
- 旗舰视频生成 **4 折扣分**优惠（15 秒视频成本 1.3-2.59 元/条）
- 支持批量任务 / 快速算力通道无需排队
- **支持 API / 智能体工作流对接** ← **这才是 CLI 真正服务的用户群**
- 旗舰模型 = Seedance 2.0（多镜头、原生音轨、超高一致性）

### 6.2.2 官方核查现状（2026-06-12）

| 调研路径 | 结果 |
|---|---|
| `jimeng.jianying.com` 官方主页 | **完全没有** Maestro / 高级会员 / 定价字样 |
| install.sh / SKILL.md / `-h` 输出 | 无 maestro 字样（已查过）|
| 服务端 error log | 仅返回 `current account is not maestro vip` 关键字 |
| **官方会员定价页** | ❌ 未访问（应该登录态下能看到，下个窗口任务）|

**结论**：Maestro VIP 价格信息**目前全部是二手聚合**，必须登录即梦网页版找官方会员页核查。**不要基于二手价格做升级决策**。

### 6.2.3 升级判断的 CTO 视角（不是建议，是判断框架）

凯戈基础会员 ~30 元/月（1080 积分）vs Maestro VIP ~649 元/月（15000 积分）：

| 场景 | 升级值不值 |
|---|---|
| 今天装机演练（8 张图卡）| ❌ 不值，月度积分用不掉 1/10 |
| 后期 000 号选题首发（1-3 条视频/月）| ⚠️ 边际，看 Seedance 2.0 vs 网页版生成质量差距 |
| K·AI 全平台稳定产出（5+ 条视频/周）| ✅ 可能值，Maestro 的 4 折扣分 + API 对接 = ROI 拉满 |
| 凯戈学生用 CLI 训练 / 实战 | ✅ 教学场景独立判断（教学预算 vs IP 预算分开核算）|

**决策时机**：等凯戈 000 号选题首发后看产出节奏，**不是现在拍板**。
...
| 项 | 优先级 | 何时补 | 前置条件 |
|---|---|---|---|
| **Maestro VIP 到底是什么等级**（即梦 Pro / 联合会员 / 独立产品）| **高** | 凯戈考虑升级会员前 | 登录即梦网页版找会员定价页 |
| **官方核查二手价格**（649/433/5199 元）| **高** | 下个窗口 | 登录态查官方定价页 |
| Maestro VIP 升级价格 vs 剪映 X 即梦联合会员（3 折弹窗）对账 | **高** | 价格核实后 | 拿到两个产品的官方价格 |
